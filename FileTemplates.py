...
class CreateFileFromTemplateCommand(sublime_plugin.WindowCommand):
            ...
            for filename in filenames:
                if filename.endswith(".file-template"):
                    ...
                    self.templates.append(
                        os.path.basename(root) + ": " + os.path.splitext(filename)[0])
    ...
    def build_relative_paths(self):
        ...
        if len(path) > 0:
            self.relative_paths = [
                "Default: " + self.template.find("path").text]

        self.relative_paths.append(
            self.ROOT_DIR_PREFIX + os.path.split(self.root)[-1] + self.ROOT_DIR_SUFFIX)

        for base, dirs, files in os.walk(self.root):
            dirs_copy = dirs[:]
            [dirs.remove(dir)
             for dir in dirs_copy if self.excluded.search(dir)]
